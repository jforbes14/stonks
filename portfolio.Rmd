...
get_prices <- function(ticker, from="1900-01-01", to=Sys.Date()) {
  ...
  extract <- data.frame(ticker = ticker, date = rownames(x), price = x$Close) %>% 
...
monthly_df <- monthly_df %>% 
  mutate(yr5_log_mean = 60*mean_log_price_ratio, yr5_log_sd = 60*sd_log_price_ratio) %>% 
  mutate(yr5_loss = pnorm(0, mean = 60*mean_log_price_ratio, sd = 60*sd_log_price_ratio),
         yr10_loss = pnorm(0, mean = 120*mean_log_price_ratio, sd = 120*sd_log_price_ratio))

monthly_df %>% View
```
```{r}
daily_df <- daily_df %>% 
  mutate(yr5_log_mean = 365*5*mean_log_price_ratio, yr5_log_sd = 365*5*sd_log_price_ratio) %>% 
  mutate(yr5_loss = pnorm(0, mean = 365*5*mean_log_price_ratio, sd = 365*5*sd_log_price_ratio),
         yr10_loss = pnorm(0, mean = 365*10*mean_log_price_ratio, sd = 365*10*sd_log_price_ratio))
